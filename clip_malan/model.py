...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ln_proj_layers_mlp = inputs[3]
        ln_proj_layers_att = inputs[4]
        ...
        att_proj = ln_proj_layers_att[counter]
        mlp_proj = ln_proj_layers_mlp[counter]

        x = x + self.attention(att_proj(self.ln_1(x), self.text_layer, counter))
        x = x + self.mlp(mlp_proj(self.ln_2(x), self.text_layer, counter))
        return [x, compound_prompts_deeper, counter, ln_proj_layers_mlp, ln_proj_layers_att]
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts,  ln_proj_layers_mlp,
        ln_proj_layers_att):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0,  ln_proj_layers_mlp,
        ln_proj_layers_att])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
