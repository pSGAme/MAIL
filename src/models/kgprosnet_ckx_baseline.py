...
class TextPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model:CLIP, device):
        ...
        prompts = ["a photo of " + name + " from " + "X "*n_ctx +"real world domain." for name in classnames] # xxxxxxxxx classname .
...
class prosnet(nn.Module):
    def __init__(self, cfg, dict_clss:dict, dict_doms:dict, device):
        ...
        self.cls_num_tokens = 300 # len(self.dict_clss)self.cfg.number_of_white
    ...
    def forward(self, image, domain_name, class_name, stage): # bt 3, 244, 244
        ...
        if self.cfg.tp_N_CTX != -1:
            ...
            text_template = torch.cat([clip.tokenize(f"a photo of a {c} from real world.") for c in class_name]).to(self.device)
