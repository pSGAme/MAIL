...
class TextPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model:CLIP, device):
        ...
        # self.teacher_ctx = torch.empty(n_ctx, self.ctx_dim, dtype=dtype).to(device)  # 16, 512
        # nn.init.normal_(self.teacher_ctx, std=0.02)
    ...
    def forward(self):
        ctx = self.ctx
        # self.teacher_ctx = 0.1 * self.teacher_ctx + 0.9 * ctx
        #teacher_ctx = self.teacher_ctx
        if ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1) # 300,16,512
            #teacher_ctx = teacher_ctx.unsqueeze(0).expand(self.n_cls, -1, -1)  # 300,16,512
        ...
        # teacher_prompts = [torch.cat([self.origin_text_embedding[i, :self.prefix_index[i]], teacher_ctx[i],
        #                       self.origin_text_embedding[i, self.prefix_index[i] + self.n_ctx:]], dim=0).view(1, -1,
        #                                                                                                       self.ctx_dim)
        #            for i in range(self.n_cls)]
        # teacher_prompts = torch.cat(teacher_prompts, dim=0)

        return prompts #teacher_prompts # 300,77,512
...
class metaClassifer(nn.Module):
    def __init__(self, cfg, dict_class, dict_doms, device):
        super().__init__()
        self.cfg = cfg
        self.device = device
        self.dict_doms = dict_doms
        self.dict_class = dict_class
        dom_names = dict_doms.keys()
        class_names = dict_class.keys()
        myclip:CLIP = self.load_clip()

        self.text_encoder = myclip.encode_text

        self.dom_classifer = []
        for dom_name in dom_names:
            text_template = torch.cat([clip.tokenize(f"a photo of a {c} from {dom_name} domain.") for c in class_names]).to(
                self.device)
            print(text_template.shape)
            with torch.no_grad():
                classifier = self.text_encoder(text_template)
            self.dom_classifer.append(classifier)
        self.dom_classifer = torch.stack(self.dom_classifer,dim=0) # dom_num, class_num, 512
        self.dom_classifer = nn.Parameter(self.dom_classifer)

    def forward(self,domain_name): # bt 3, 244, 244
        dom_id = utils.numeric_classes(domain_name, self.dict_doms)
        return  self.dom_classifer[dom_id]
    def load_clip(self):
...
class prosnet(nn.Module):
    def __init__(self, cfg, dict_clss:dict, dict_doms:dict, device):
        super().__init__()
        #self.classifier = metaClassifer(cfg, dict_clss, dict_doms, device)
        self.cfg = cfg
        self.device = device
        ...
        self.cls_num_tokens =  len(self.dict_clss) # self.cfg.number_of_white
        #self.cls_num_tokens = self.cfg.number_of_white
        ...
        if self.cfg.tp_N_CTX != -1: # text prompt numbers # 16
            if self.cfg.use_NTP == 1: # all text prompt tuning # 0
                self.text_prompt_learner = PromptLearner(self.cfg, self.dict_clss.keys(),clip, device)
            else :
                self.text_prompt_learner = TextPromptLearner(self.cfg, self.dict_clss.keys(), clip, device) # look here
            self.text_encoder = TextEncoder(clip)
            self.tokenized_prompts = self.text_prompt_learner.tokenized_prompts
        else :
            self.text_encoder = clip.encode_text
        if self.cfg.DOM_PROJECT > -1: # == -1
    ...
    def incorporate_prompt(self, x, dom_index, cls_index, stage, dom_prompts=None, cls_prompts=None):
        ...
        if stage == 1:
            ...
            x = x.reshape(x.shape[0], x.shape[1], -1) # 400 768 49
            x = x.permute(0, 2, 1) # 400 49 768
            # nn.init.uniform_(self.specific_domain_prompts.data, -self.val, self.val)
            # nn.init.uniform_(self.specific_class_prompts.data, -self.val, self.val)

            sp_dom_prompts = self.sp_dom_prompt_proj(self.specific_domain_prompts) # dom_num, 768
            sp_cls_prompts = self.sp_cls_prompt_proj(self.specific_class_prompts) # cls_num, 768
            ...
            # print(cls_index)
            # print(dom_index)
            cls_prompt_mask[range(B), cls_index, :] = 1 # batchsize, dom_num, 768
            dom_prompt_mask[range(B), dom_index, :] = 1 # batchsize, dom_num, 768
            sp_cls_prompts = sp_cls_prompts.expand(B,-1,-1).masked_fill(cls_prompt_mask, 0) # batchsize, dom_num, 768 768
            sp_dom_prompts = sp_dom_prompts.expand(B,-1,-1).masked_fill(dom_prompt_mask, 0) # batchsize, dom_num,
            ...
            x = torch.cat((
            (self.feature_template+self.clip_positional_embedding[0]).expand(B,-1).view(B, 1, -1),
             dom_prompts,  #self.ge_dom_prompt_template_2.expand(B, -1, -1) +
            cls_prompts, #self.ge_cls_prompt_template_2.expand(B, -1, -1) +
            x + self.clip_positional_embedding[1:]
        ), dim=1)
            ...
            x = self.conv1(x)  #    self.conv1 = clip.visual.conv1
            # print(x.shape) # 400, 768, 7, 7
            x = x.reshape(x.shape[0], x.shape[1], -1)  # 400 768 49
            x = x.permute(0, 2, 1)  # 4
            x = torch.cat((
    ...
    def vit(self, x, out_token):
        ...
        if out_token <= 1:
            x = self.ln_pre(x) # clip.visual.ln_pre
            x = x.permute(1, 0, 2)
            x = self.transformer(x) # clip.visual.transformer
            x = x.permute(1, 0, 2)
    ...
    def image_encoder(self, image, dom_id, cls_id, stage):
        if stage == 1:
            x = self.incorporate_prompt(image, dom_id, cls_id, 1)
            x = self.vit(x, 0)
        elif stage == 2: # input: template + specific prompts train
    ...
    def forward(self, image, domain_name, class_name, stage): # bt 3, 244, 244
        ...
        image_features_raw = self.image_encoder(image, dom_id, cls_id, 5)
        if self.cfg.tp_N_CTX != -1:
            text_prompts = self.text_prompt_learner()
            #print(text_prompts.shape)
            tokenized_prompts = self.tokenized_prompts
            text_features = self.text_encoder(text_prompts, tokenized_prompts)
            # with torch.no_grad():
            #     teacher_features = self.text_encoder(teacher_prompts, tokenized_prompts)
            # print(text_features.shape) # [300, 512]
            # print(tokenized_prompts[:,:10])
        else :
            text_template = torch.cat([clip.tokenize(f"a photo of a {c}.") for c in self.dict_clss.keys()]).to(self.device)
        ...
        # teacher_features = teacher_features / text_features.norm(dim=-1, keepdim=True)

        # logit_scale = self.logit_scale.exp()
        # logits = logit_scale * image_features @ text_features.t()
        return image_features, image_features_raw, text_features,
