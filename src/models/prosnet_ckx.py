...
class prosnet(nn.Module):
    def __init__(self, cfg, dict_clss:dict, dict_doms:dict, device):
        ...
        self.cls_num_tokens = len(self.dict_clss) # len(self.dict_clss)self.cfg.number_of_white
        ...
        self.text_generator_txt2img = nn.Parameter(scale * torch.randn(512, 768))
        self.text_generator = VisionTransformer(224, 32, 768, self.cfg.generator_layer, 12, 512)
        self.text_generator_proj = nn.Parameter(scale * torch.randn(width, 768)) # 768->768 作为LP来用
    ...
    def sqrtvar(selfself, x):
        t = (x.var(dim=1, keepdim=True) + 1e-6).sqrt() # b, 1
        # t = t.repeat(1,x.shape[1]) # b, cls
        return  t

    def dsu(self, input):
        #input_shape: [b, cls/dom num, 768]
        #mean = input.mean(dim=2, keepdim=False) # b, cls
        std = (input.var(dim=1, keepdim=True) + 1e-6).sqrt()  # b, 1, 768
        std = std.repeat(1, input.shape[1], 1) # b, cls, 768
        beta = self._reparameterize(input, std)
        return beta


    def incorporate_prompt(self, x, dom_index, cls_index, stage, dom_prompts=None, cls_prompts=None, txt_prompts=None):
        B = x.shape[0]
        ...
        if stage == 1:
            ...
            # # no mask
            # sp_cls_prompts = sp_cls_prompts.expand(B,-1,-1) # batchsize, dom_num, 768 768
            # sp_dom_prompts = sp_dom_prompts.expand(B,-1,-1)# batchsize, dom_num,
            ...
            txt_prompts.view(B, self.cfg.tp_N_CTX, -1),
            x + self.clip_positional_embedding[1:]
        ), dim=1)
            ...
            x = self.text_generator.conv1(x)
            x = x.reshape(x.shape[0], x.shape[1], -1) # 65 768 49
            x = x.permute(0, 2, 1) # 65 49 768
            x = torch.cat((
                self.text_prompt_learner.ctx.expand(B, -1, -1)@ self.text_generator_txt2img,
                x + self.text_generator.positional_embedding[1:]
            ), dim=1)
    ...
    def vit2(self, x, out_token):
        ...
        if out_token == 1:
            ...
            x = self.text_generator.ln_pre(x)
            x = x.permute(1, 0, 2)
            x = self.text_generator.transformer(x)
            x = x.permute(1, 0, 2)
            x = self.text_generator.ln_post(x[:, :out_token, :])
            x = x @ self.text_generator_proj
        return x
    ...
    def image_encoder(self, image, dom_id, cls_id, stage):
        if stage == 1: # training for specific prompts
            ...
            txt_x = self.incorporate_prompt(image, dom_id, cls_id, 5)
            txt_x = self.vit2(txt_x, self.cfg.tp_N_CTX)
            ...
            x = self.incorporate_prompt(image, dom_id, cls_id, 3, x[:,:self.cfg.GP_DOM_NUM_TOKENS], x[:,self.cfg.GP_DOM_NUM_TOKENS:], txt_x) # cat CLS generated prompts + image patch
            x = self.vit(x, 1)
            ...
            x = self.incorporate_prompt(image, dom_id, cls_id,3, x[:,:self.cfg.GP_DOM_NUM_TOKENS], x[:,self.cfg.GP_DOM_NUM_TOKENS:], txt_x)
            x = self.vit(x, 1)
  ...
  # #random mask!
            # p1 = 0.005
            # p2 = 0.2
            # my_mask_cls = torch.rand(B, sp_cls_prompts.shape[0]).cuda() # batch size, class_num, (1)
            # my_mask_dom = torch.rand(B, sp_dom_prompts.shape[0]).cuda() # batch size, dom_num, (1)
            # my_mask_cls = my_mask_cls.unsqueeze(2).repeat(1, 1, sp_cls_prompts.shape[1]) > p1 # batch size, class_num, 768
            # my_mask_dom = my_mask_dom.unsqueeze(2).repeat(1, 1, sp_dom_prompts.shape[1]) > p2# batch size, class_num, 768
            # sp_cls_prompts = sp_cls_prompts.expand(B, -1, -1) * my_mask_cls
            # sp_dom_prompts = sp_dom_prompts.expand(B, -1, -1) * my_mask_dom
            # print(sp_dom_prompts[0])