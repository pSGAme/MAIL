...
def my_mean_average_precision(database_hash: torch.FloatTensor, database_hash2: torch.FloatTensor,
                              test_hash: torch.FloatTensor, test_hash2: torch.FloatTensor,
                           database_labels: torch.BoolTensor, test_labels: torch.BoolTensor, K):
    AP_200 = list()
    AP_all = list()
    Precision_100 = list()
    Precision_200 = list()
    ...
    count = 0
    ranklist_per_class = {}
    for queryX, queryX2, queryLabels in tqdm(zip(test_hash, test_hash2, test_labels), desc='Caluate mAP', total=len(
            test_hash)):  # tqdm(zip(test_hash, test_labels), leave=False, desc='MAP', ncols=50, bar_format="{desc}|{bar}|{percentage:3.0f}% {elapsed}"):
        thisAP_200, thisAP_all, thisPrecision_100, thisPrecision_200, ids = _my_partedMAP_all(database_hash, database_hash2,
                                                                                           queryX[None, :], queryX2[None, :],
                                                                                           database_labels,
                                                                                           queryLabels[None, :], K)
...
def _my_partedMAP_all(database_hash: torch.FloatTensor, database_hash2: torch.FloatTensor,
                      test_hash: torch.FloatTensor, test_hash2: torch.FloatTensor,
                      database_labels: torch.BoolTensor, test_labels: torch.BoolTensor, K):  # R = 1000
    R = K

    # [1, Nb]
    sim = -((test_hash - database_hash) ** 2).sum(-1)
    sim = sim.reshape(-1)
    sim2 = -((test_hash2 - database_hash2) ** 2).sum(-1)
    sim2 = sim2.reshape(-1)
    sim = sim + sim2
    # [Nq, R] top R queried from base
