...
from src.utils.mAP import mean_average_precision, my_mean_average_precision
...
def my_compute_retrieval_metrics(query, query_similarity, query_class, database, database_similarity, database_class):
    # query = query / query.norm(dim=-1, keepdim=True)
    # database = database / database.norm(dim=-1, keepdim=True)
    query_similarity = query_similarity / query_similarity.norm(dim=-1, keepdim=True)
    database_similarity = database_similarity / database.norm(dim=-1, keepdim=True)
    ...
    ap200, ap_all, prec_100, prec200, apPerclass, ranklist_per_class = my_mean_average_precision(database,  database_similarity,
                                                                                              query,query_similarity,
                                                                                              database_class,
                                                                                              query_class, 200)

    return {'mAP@200': ap200, 'prec@200': prec200, 'mAP@all': ap_all, 'prec@100': prec_100, 'class_ap': apPerclass,
            'ranklist': ranklist_per_class}
