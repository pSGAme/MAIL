...
class Trainer:
    ...
    def __init__(self, args):
        ...
        # log_file = os.path.join(self.path_cp, args.log_name + "_num_shots=" + str(args.num_shots) +
        #                          "_ivlp_end: " + str(args.ivlu_end_layer) + " _mail_end: " + str(args.end_layer) + ".txt")
        # #log_file = os.path.join(self.path_cp, args.log_name + "_num_shots=" + str(args.num_shots) + "_No ln.txt")
    ...
    def training_set(self):
        ...
        if self.args.optimizer=='sgd':
            optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), weight_decay=self.args.l2_reg, momentum=self.args.momentum, nesterov=False, lr=lr)
            ...
            optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=self.args.l2_reg)
        print("===============================================")
        return optimizer
            ...
            if self.args.dataset=='DomainNet':
                ...
                for includeSeenClassinTestGallery in [0, 1]:
                    ...
                    result_unnorm,  result_norm\
                        = evaluate(te_loader_query, te_loader_gallery, self.model, self.te_dict_class, self.dict_doms, 4, self.args)
