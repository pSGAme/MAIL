    ...
    def __init__(self, args):
        ...
        if args.maple:
            log_file = os.path.join(self.path_cp, args.log_name + "_num_shots=" + str(args.num_shots) +
                                    "_depth" + str(args.maple_depth) + "_length" + str(args.maple_length) + "_trick"
                                    + str(args.trick)+".txt")
        else:
            log_file = os.path.join(self.path_cp, args.log_name + "_num_shots=" + str(args.num_shots) +
                                    "_imagedepth" + str(args.visualDepth) + "_imagelength" + str(args.visualNumTokens) +
                                    "_textdepth" + str(args.textDepth) + "_textlength" + str(args.textNumTokens) + "_trick"
                                    + str(args.trick) +".txt")
...
def evaluate(loader_sketch, loader_image, model, dict_clss, dict_doms, stage, args):
    ...
    for i, (sk, cls_sk, dom) in tqdm(enumerate(loader_sketch), desc='Extrac query feature', total=len(loader_sketch)):
        ...
        prompts, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision = model.text_prompt_learner()

        # text_features = model.text_encoder(prompts, tokenized_prompts, deep_compound_prompts_text)
        sk_em = model.visual_encoder(sk, shared_ctx, deep_compound_prompts_vision)
    ...
    for i, (im, cls_im, dom) in tqdm(enumerate(loader_image), desc='Extrac gallery feature', total=len(loader_image)):
        ...
        # text_features = model.text_encoder(prompts, tokenized_prompts, deep_compound_prompts_text)
        im_em = model.visual_encoder(im, shared_ctx, deep_compound_prompts_vision)
