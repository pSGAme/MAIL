...
class Trainer:
    ...
    def __init__(self, args):
        ...
        self.clip_model, self.preprocess = clip.load(self.args.clip_backbone)
    ...
    def training_set(self):
        ...
        if self.args.optimizer == 'sgd':
            optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.clip_model.parameters()),
            ...
            optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.clip_model.parameters()), lr=lr,
            ...
            if self.args.dataset == 'DomainNet':
                ...
                for includeSeenClassinTestGallery in [0, 1]:
                    ...
                    result_unnorm, result_norm \
                        = evaluate(te_loader_query, te_loader_gallery, self.clip_model, self.te_dict_class, self.dict_doms,
                                   4, self.args)
                ...
                te_loader_gallery = DataLoader(dataset=data_te_gallery, batch_size=self.args.batch_size * 5,
                                               ...
                                               num_workers=self.args.num_workers, pin_memory=True)
                result_unnorm, result_norm \
                    = evaluate(te_loader_query, te_loader_gallery, self.clip_model, self.te_dict_class, self.dict_doms, 4,
                               self.args)
    ...
    def resume_from_checkpoint(self, resume_dict):
        if resume_dict is not None:
            ...
            self.clip_model.load_state_dict(checkpoint['model_state_dict'], strict=False)
    ...
    def do_epoch_with_triplet(self, optimizer, current_epoch):
        ...
        for i, (im, cls, dom) in enumerate(train_loader):
            ...
            with torch.no_grad():
                class_embeddings = self.clip_model.encode_text(tokenized_prompts).to(device)

            soft_label = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)
            feature = self.clip_model.visual(im)
