...
class Trainer:
    ...
    def training_set(self):
        ...
        train_parameters = ['text_prompt_learner', 'text_encoder.text_projection',
                            'visual_encoder.proj',
                            'visual_encoder.prompt_embeddings', ]  # 'visual_encoder.class_embedding'
    ...
    def do_epoch_with_triplet(self, optimizer, current_epoch):
        ...
        for i, (im, cls, dom) in enumerate(train_loader):
            ...
            hard_labels = cls_numeric
            loss_cls_all = torch.tensor(0.0).to(device)
            loss_triplet_all = torch.tensor(0.0).to(device)

            j = 0

            loss_triplet, co2 = triplet_loss(features, hard_labels)
            #print(loss_triplet)
            loss_triplet_all += loss_triplet
            #print(loss_triplet_all)

            for feature, soft_label in zip(features, soft_label_list):
                feature = feature.unsqueeze(0)
                hard_label = hard_labels[j:j+1]
                loss_cls, co = soft_sup_con_loss(feature, soft_label, hard_label, device=device)
                correct += co
            ...
            loss_triplets.update(loss_triplet_all.item(), im.size(0))
