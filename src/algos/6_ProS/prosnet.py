...
class prosnet(nn.Module):
    ...
    def incorporate_prompt(self, x, dom_index, cls_index, stage, dom_prompts=None, cls_prompts=None):
        ...
        if stage == 1:
            ...
            # # mask
            sp_cls_prompts = sp_cls_prompts.expand(B, -1, -1).masked_fill(cls_prompt_mask,
                                                                          0)  # batchsize, dom_num, 768 768
    ...
    def image_encoder(self, image, dom_id, cls_id, stage):
        if stage == 1:
            ...
            x = self.incorporate_prompt(image, dom_id, cls_id, 3, x[:, :self.cfg.GP_DOM_NUM_TOKENS],
                                        x[:, self.cfg.GP_DOM_NUM_TOKENS:])  # cat CLS generated prompts + image patch
            x = self.vit(x, 1)
        elif stage == 4:
            x = self.incorporate_prompt(image, dom_id, cls_id, 4)
            x = self.vit(x, self.cfg.GP_DOM_NUM_TOKENS + self.cfg.GP_CLS_NUM_TOKENS)
            x = self.incorporate_prompt(image, dom_id, cls_id, 3, x[:, :self.cfg.GP_DOM_NUM_TOKENS],
                                        x[:, self.cfg.GP_DOM_NUM_TOKENS:])
            x = self.vit(x, 1)
