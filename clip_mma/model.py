...
class ModifiedResidualAttentionBlock_for_FLOPS(nn.Module):
    def __init__(self, d_model: int, n_head: int, layer_index: int, attn_mask: torch.Tensor = None):
        ...
        self.adapter = nn.Sequential(OrderedDict([
            ("down", nn.Sequential(nn.Linear(d_model, 32), nn.ReLU())),
            ("up", nn.Linear(32, d_model))
        ]))
    ...
    def forward(self, x):
        ...
        if isinstance(x, List):
            ...
            if seq_adapter is not None:
                ...
                y = self.ln_1(y)
                y = self.adapter.down(y)
                # if shared_adapter is not None:
                #     y = self.seq(y)
                y = self.adapter.up(y) * scale
