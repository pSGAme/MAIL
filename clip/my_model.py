...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, y = None):
    ...
    def forward(self, x: torch.Tensor, y=None):
        if y is None:
            ...
            # x = x + self.ln_1(self.attention(x, y))
...
class twoLayerResidualAttentionBlock(nn.Module):
    def __init__(self, width: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.residual1 = ResidualAttentionBlock(width, heads, attn_mask)
        self.residual2 = ResidualAttentionBlock(width, heads, attn_mask)

    def forward(self, my_input):
        x = my_input[0]
        y = my_input[1].unsqueeze(1).repeat(1, x.shape[1], 1)
       # print(x.shape, y.shape)
        return self.residual2(self.residual1(x, y))
...
class Cross_Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[twoLayerResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers)])

    def forward(self, x: torch.Tensor, y):
        # print(x.shape, y.shape) # sequential 只接受单输入:)
        return self.resblocks((x,y))


class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        # 224, 32, 768, 12, 12, 512
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
