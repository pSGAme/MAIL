...
class Convpass(nn.Module):
    def __init__(self, outdim=768, dim=8, xavier_init=False):
        super().__init__()

        self.a = torch.nn.Parameter(torch.ones(outdim))
        self.b = torch.nn.Parameter(torch.zeros(outdim))
        ...
        # self.adapter_down = nn.Linear(outdim, dim)  # equivalent to 1 * 1 Conv
        # self.adapter_up = nn.Linear(dim, outdim)  # equivalent to 1 * 1 Conv
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        #self.ln_1_update = LayerNorm(d_model)
        ...
        #self.ln_2_update = LayerNorm(d_model)

        #self.adapter_1 = Convpass(outdim=d_model)
        #self.adapter_2 = Convpass(outdim=d_model)
...
def build_model(state_dict: dict):
    ...
    for name, params in model.named_parameters():
        if "_update" in name:
            ori_name = name.replace("_update", "") # remove update xD
            with torch.no_grad():
                #print(ori_name)
                # if ori_name in state_dict.keys():
                #     # print("you are right")
                params.copy_(state_dict[ori_name])
    return model.eval()
